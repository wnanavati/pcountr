...
test_that("read_site loads all 20 CNT files with NA depths", {
  site <- suppressWarnings(read_site(extdata(), quiet = TRUE))
...
test_that("depth from sheet is attached to CNT samples", {
  ...
  write_sheet(tmp, data.frame(
    file         = c("LMSH001", "LMSH009"),
    depth_top    = c(10.0, 20.0),
    depth_bottom = c(11.0, 21.0),
    stringsAsFactors = FALSE
  ))
  site <- suppressWarnings(
    read_site(extdata(), metadata = tmp, quiet = TRUE)
  )
  expect_equal(site$samples[["LMSH001"]]$meta$depth_top,    10.0)
  expect_equal(site$samples[["LMSH001"]]$meta$depth_bottom, 11.0)
  expect_equal(site$samples[["LMSH009"]]$meta$depth_top,    20.0)
})
...
test_that("col_map translates custom column names", {
  ...
  site <- suppressWarnings(
    read_site(extdata(), metadata = tmp,
              col_map = c(file = "SampleID", depth_top = "Top_cm",
                          depth_bottom = "Bot_cm"),
              quiet = TRUE)
  )
  expect_equal(site$samples[["LMSH001"]]$meta$depth_top, 15.5)
})
...
test_that("samples with depths sort before samples without", {
  ...
  write_sheet(tmp, data.frame(
    file         = c("LMSH046", "LMSH001", "LMSH017"),
...
test_that("sheet row with no matching file produces a warning", {
  ...
  on.exit(unlink(tmp))
  write_sheet(tmp, data.frame(
    file         = c("LMSH001", "DOESNOTEXIST"),
    depth_top    = c(10.0, 50.0),
    depth_bottom = c(11.0, 51.0),
    stringsAsFactors = FALSE
  ))
  expect_warning(
    read_site(extdata(), metadata = tmp, quiet = TRUE),
...
test_that("YAML embedded depth is preserved through read_site", {
  ...
  file.copy(extdata("ECG.DIC"), file.path(tmp_dir, "ECG.DIC"))
  write_pollen_count(cnt, file.path(tmp_dir, "LMSH001.yaml"))

  loaded <- read_site(tmp_dir, quiet = TRUE)
  expect_equal(loaded$samples[["LMSH001"]]$meta$depth_top,    12.5)
  expect_equal(loaded$samples[["LMSH001"]]$meta$depth_bottom, 13.5)
})
...
test_that("real LM_depths.txt loads with col_map and correct depths", {
  sheet_path <- extdata("LM_depths.txt")
  # 55 rows in sheet, 20 CNT files in extdata -> 35 sheet rows warn "no file"
  site <- suppressWarnings(
    read_site(extdata(), metadata = sheet_path,
              col_map = c(file = "SampleName",
                          depth_top    = "DepthTop",
                          depth_bottom = "DepthBottom"),
              quiet = TRUE)
  )
  # All 20 extdata samples should now have depths
  tops <- vapply(site$samples, function(s) s$meta$depth_top, NA_real_)
  expect_true(all(!is.na(tops)))
  # Check a few known values
  expect_equal(site$samples[["LMSH001"]]$meta$depth_top,     0.0)
  expect_equal(site$samples[["LMSH001"]]$meta$depth_bottom,  0.5)
  expect_equal(site$samples[["LMSH158"]]$meta$depth_top,    44.0)
  expect_equal(site$samples[["LMSH158"]]$meta$depth_bottom, 44.25)
  # Samples should be sorted shallowest first
  expect_equal(names(site$samples)[1],  "LMSH001")
  expect_equal(names(site$samples)[20], "LMSH158")
})

test_that("real LM_depths.txt sheet warns about rows with no matching file", {
  sheet_path <- extdata("LM_depths.txt")
  expect_warning(
    read_site(extdata(), metadata = sheet_path,
              col_map = c(file = "SampleName",
...
test_that("set_metadata updates depth and re-sorts by name", {
  ...
  site <- set_metadata(site, "LMSH001", depth_top = 10.0, depth_bottom = 11.0)
  expect_equal(site$samples[["LMSH001"]]$meta$depth_top,    10.0)
  expect_equal(site$samples[["LMSH001"]]$meta$depth_bottom, 11.0)
  # LMSH001 should now be first (only sample with a depth)
  expect_equal(names(site$samples)[1], "LMSH001")
})

test_that("set_metadata re-sorts correctly when multiple depths set", {
  site <- suppressWarnings(read_site(extdata(), quiet = TRUE))
  site <- set_metadata(site, "LMSH046", depth_top = 40.0, depth_bottom = 41.0)
  site <- set_metadata(site, "LMSH001", depth_top = 10.0, depth_bottom = 11.0)
  site <- set_metadata(site, "LMSH017", depth_top = 25.0, depth_bottom = 26.0)
...
test_that("set_metadata errors on invalid interval", {
  site <- suppressWarnings(read_site(extdata(), quiet = TRUE))
  expect_error(
    set_metadata(site, "LMSH001", depth_top = 20.0, depth_bottom = 10.0),
...
test_that("set_metadata with age updates age fields", {
  site <- suppressWarnings(read_site(extdata(), quiet = TRUE))
  site <- set_metadata(site, "LMSH001",
                       depth_top = 10.0, depth_bottom = 11.0,
                       age_top = 500.0, age_bottom = 600.0)
  expect_equal(site$samples[["LMSH001"]]$meta$age_top,    500.0)
  expect_equal(site$samples[["LMSH001"]]$meta$age_bottom, 600.0)
})
...
test_that("set_metadata updates sample_name, quantity, spike fields", {
  ...
  site <- set_metadata(site, "LMSH001",
                       depth_top       = 0.0,
                       depth_bottom    = 1.0,
                       sample_name     = "LM23sh#001",
                       sample_quantity = 2.5,
                       sample_units    = "ml",
                       spike_tablets   = 2L,
                       spike_density   = 9666,
                       spike_units     = "tablets")
  m <- site$samples[["LMSH001"]]$meta
  expect_equal(m$sample_name,     "LM23sh#001")
...
test_that("set_metadata omitted fields leave existing values unchanged", {
  ...
  expect_equal(site$samples[["LMSH001"]]$meta$depth_top, 0.0)
  expect_true(is.na(site$samples[["LMSH001"]]$meta$age_top))
  expect_equal(site$samples[["LMSH001"]]$meta$sample_quantity, 1.5)
})
